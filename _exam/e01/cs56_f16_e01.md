...
If you were asked at a job interview to briefly describe the purpose of `ant` and the `buidl.xml` file, what would you say?  Same comment as above about the "level of detail".
...
(10 pts) Briefly how the `.equals()` method invoked in this case (with no overloaded `.equals` method in `Dog`) determines whether to return `true` or `false`, *both* in this specific case, *and* in general. (Both parts needed for full credit).
