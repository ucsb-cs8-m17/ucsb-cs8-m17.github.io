...
<li class="page-break-before" markdown="1">Please refer to the [handout](handout) that came with this exam, and find the example `pytest` test cases from lab03.
...
<ol>
<li markdown="1" style="margin-bottom:15em;"> (5 pts) In the space below, write a stub (and only the stub) for a function called `smallestOdd` that takes a parameter called `alist` which should be a non-empty list of integers.  The function should return the smallest odd integer.
</li>
...
|-----|---------------|-----|-----|-----|-----|--------|----------------------------|-------------------|
| (0 pts) |`indexSmallest_a(['ant','dog','bear','horse'])` | &#x2714; | | | | | | |
| (3 pts)   | `indexSmallest_a(['tiger','cow','cat','monkey'])` | | | | | | | |
| (3 pts)   | `indexSmallest_a(['rat','pig','fish','ox'])` | | | | | | | | 

| Points | Function Call | `0` | `1` | `2` | `3` | `None` | Python<br>error<br>message | something<br>else |
|-----|---------------|-----|-----|-----|-----|--------|----------------------------|-------------------|
| (3 pts) | `indexSmallest_b(['ant','dog','bear','horse'])`    | | | | | | | |
| (3 pts) | `indexSmallest_b(['tiger','cow','cat','monkey'])` | | | | | | | |
| (3 pts) | `indexSmallest_b(['rat','pig','fish','ox'])`      | | | | | | | | 

| Points | Function Call | `0` | `1` | `2` | `3` | `None` | Python<br>error<br>message | something<br>else |
|-----|---------------|-----|-----|-----|-----|--------|----------------------------|-------------------|
| (3 pts) | `indexSmallest_c(['ant','dog','bear','horse'])`    | | | | | | | |
| (3 pts) | `indexSmallest_c(['tiger','cow','cat','monkey'])` | | | | | | | |
| (3 pts) | `indexSmallest_c(['rat','pig','fish','ox'])`      | | | | | | | | 



| Points | Function Call | `0` | `1` | `2` | `3` | `None` | Python<br>error<br>message | something<br>else |
|-----|---------------|-----|-----|-----|-----|--------|----------------------------|-------------------|
| (3 pts) | `indexSmallest_d(['ant','dog','bear','horse'])`    | | | | | | | |
| (3 pts) | `indexSmallest_d(['tiger','cow','cat','monkey'])` | | | | | | | |
| (3 pts) | `indexSmallest_d(['rat','pig','fish','ox'])`      | | | | | | | | 
